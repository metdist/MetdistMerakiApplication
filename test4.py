...
st.markdown(
    ...
    /* Center the devices display container */
    .centered-container {
        display: flex;
...
def main():
    ...
    if st.session_state['logged_in']:
        ...
        # Display Devices in Selected Network
        ...
        # Calculate the count of devices in each status
        status_counts = devices['status'].value_counts()

        # Display the devices in a centered container
        
        for network_id, devices in devices.groupby('networkId'):
            if network_id == selected_network_id:
                styled_devices = devices[['name', 'serial', 'mac','productType','lastReportedAt', 'lanIp', 'status']].style.applymap(coloured_status, subset=['status'])
                network_devs_statuses = devices['status'].value_counts()
                # Center the devices table

                # Function to display color-coded bars for status counts
                def display_status_counts(status_counts):
                    st.write(f"### Devices Status for {selected_network_name} Network")

                    # Define color mapping for each status
                    colors = {
                        "online": "#4CAF50",   # Green
                        "offline": "#FF5252",  # Red
                        "alerting": "#FFC107", # Amber
                        "dormant": "#9E9E9E"   # Gray
                    }

                    # HTML template for displaying colored bars
                ...
                # Call this function to display device status counts with colored bars
